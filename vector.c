...
void init(Vector *vector, int memorySize) {
    ...
    (*vector).size = 0;
    (*vector).array = (int*)malloc(memorySize);
}
...
int insert(Vector *vector, int location, int value) {
    ...
    //determine future size.
    int future_mem_size = sizeof(value)+sizeof(*vector);

    //If future size is bigger
    if(future_mem_size>(*vector).memorySize){
        ...
        int* newArray = (int*)malloc((*vector).memorySize);
